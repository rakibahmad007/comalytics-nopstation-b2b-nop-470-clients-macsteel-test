...
namespace Nop.Plugin.Payments.PayGate.Controllers
{
    public class PaymentPayGateController : BasePaymentController
    {
        ...
        public async Task<IActionResult> PayGateReturnHandler(IFormCollection form)
        {
            ...
            /**
             * Enable redirect if IPN is disabled
             */
            if (_payGatePaymentSettings.DisableIpn)
            {
                ...
                if (verified)
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        foreach(var value in queryData)
                        {
                            sBuilder.Append(value.Value);
                        }
                        queryData["CHECKSUM"] = new PayGateHelper().CalculateMD5Hash(sBuilder.ToString() + encryptionKey);
                        ...
                        if (!isqueried && cnt < 5)
                        {
                            ...
                            var response = await client.PostAsync("https://secure.paygate.co.za/payweb3/query.trans", queryContent);
                            var queryResponse = await response.Content.ReadAsStringAsync();
                            await _logger.InformationAsync("Query response: " +  queryResponse + " cnt=" + cnt);
                            var dictionaryResponse =  
        ...
        public async Task<IActionResult> Configure()
        {
            ...
            model.DisableIpn = payGatePaymentSettings.DisableIpn;
            model.ActiveStoreScopeConfiguration = storeScope;
            model.UseSSL = payGatePaymentSettings.UseSSL;
            model.EnableCreditCard = payGatePaymentSettings.EnableCreditCard;
            model.EnableBankTransfer = payGatePaymentSettings.EnableBankTransfer;
            model.EnableZapper = payGatePaymentSettings.EnableZapper;
            model.EnableSnapscan = payGatePaymentSettings.EnableSnapscan;
            model.EnablePaypal = payGatePaymentSettings.EnablePaypal;
            model.EnableMobicred = payGatePaymentSettings.EnableMobicred;
            model.EnableMomoPay = payGatePaymentSettings.EnableMomoPay;
            model.EnableScanToPay = payGatePaymentSettings.EnableScanToPay;
            model.EnableSamsungPay = payGatePaymentSettings.EnableSamsungPay;
            model.EnableApplePay = payGatePaymentSettings.EnableApplePay;
            model.EnableRcsPay = payGatePaymentSettings.EnableRcsPay;
            ...
            if (storeScope > 0)
            {
                ...
                model.DisableIpn_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.DisableIpn, storeScope);
                model.UseSSL_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.UseSSL, storeScope);
                
                model.EnableCreditCard_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableCreditCard, storeScope);
                
                model.EnableBankTransfer_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableBankTransfer, storeScope);
                
                model.EnableZapper_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableZapper, storeScope);
                
                model.EnableSnapscan_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableSnapscan, storeScope);
                
                model.EnablePaypal_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnablePaypal, storeScope);
                
                model.EnableMobicred_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableMobicred, storeScope);
                
                model.EnableScanToPay_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableScanToPay, storeScope);
                
                model.EnableMomoPay_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableMomoPay, storeScope);
                
                model.EnableSamsungPay_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableSamsungPay, storeScope);
                
                model.EnableApplePay_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableApplePay, storeScope);
                
                model.EnableRcsPay_OverrideForStore = await _settingService.SettingExistsAsync(payGatePaymentSettings, x => x.EnableRcsPay, storeScope);
            }
        ...
        public async Task<IActionResult> Configure(ConfigurationModel model)
        {
            ...
            payGatePaymentSettings.DisableIpn = model.DisableIpn;
            payGatePaymentSettings.UseSSL = model.UseSSL;

            payGatePaymentSettings.EnableCreditCard = model.EnableCreditCard;
            payGatePaymentSettings.EnableBankTransfer = model.EnableBankTransfer;
            payGatePaymentSettings.EnableZapper = model.EnableZapper;
            payGatePaymentSettings.EnableSnapscan = model.EnableSnapscan;
            payGatePaymentSettings.EnablePaypal = model.EnablePaypal;
            payGatePaymentSettings.EnableMobicred = model.EnableMobicred;
            payGatePaymentSettings.EnableMomoPay = model.EnableMomoPay;
            payGatePaymentSettings.EnableScanToPay = model.EnableScanToPay;
            payGatePaymentSettings.EnableSamsungPay = model.EnableSamsungPay;
            payGatePaymentSettings.EnableApplePay = model.EnableApplePay;
            payGatePaymentSettings.EnableRcsPay = model.EnableRcsPay;
            ...
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.DisableIpn, model.DisableIpn_OverrideForStore, storeScope, false);
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.UseSSL, model.UseSSL_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableCreditCard, model.EnableCreditCard_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableBankTransfer, model.EnableBankTransfer_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableZapper, model.EnableZapper_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableSnapscan, model.EnableSnapscan_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnablePaypal, model.EnablePaypal_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableMobicred, model.EnableMobicred_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableMomoPay, model.EnableMomoPay_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableScanToPay, model.EnableScanToPay_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableSamsungPay, model.EnableSamsungPay_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableApplePay, model.EnableApplePay_OverrideForStore, storeScope, false);
            
            await _settingService.SaveSettingOverridablePerStoreAsync
                 (payGatePaymentSettings, x => x.EnableRcsPay, model.EnableRcsPay_OverrideForStore, storeScope, false);

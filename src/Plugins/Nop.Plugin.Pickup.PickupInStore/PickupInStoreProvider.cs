...
using Nop.Plugin.Pickup.PickupInStore.Areas.Admin.Components;
...
public class PickupInStoreProvider : BasePlugin, IPickupPointProvider, IWidgetPlugin
{
    ...
    public async Task<IList<string>> GetWidgetZonesAsync()
    {
        return new List<string> {
                ERPIntegrationCoreDefaults.B2BAdminWidgetZonesPickupPointsInSalesOrgDetailsBlock
            };
    }

    public Type GetWidgetViewComponent(string widgetZone)
    {
        if (widgetZone.Equals(ERPIntegrationCoreDefaults.B2BAdminWidgetZonesPickupPointsInSalesOrgDetailsBlock))
            return typeof(B2BSalesOrgPickUpPointsAdminViewComponent);

...
CREATE OR ALTER PROCEDURE [dbo].[ErpProductLoadAllPagedNopAjaxFilters] (
...
BEGIN
    ...
    -- b2bb2c custom code start
    --filter by b2b facet filter
    IF @IsB2BAccount = 1
    BEGIN
        SET @PreFilterFacetSpecIds = isnull(@PreFilterFacetSpecIds, '')    
        SET @SpecialExcludeSpecIds = isnull(@SpecialExcludeSpecIds, '')
        ...
        --Special Exclude start
		CREATE TABLE #SpecialExcludeSpecIds
		(
			SpecificationAttributeOptionId int not null
		)
		CREATE TABLE #SpecialExcludedProductsIds
		(
			ProductId int not null
		)
		INSERT INTO #SpecialExcludeSpecIds (SpecificationAttributeOptionId)
		SELECT data FROM [nop_splitstring_to_table](@SpecialExcludeSpecIds, ',')

		INSERT INTO #SpecialExcludedProductsIds (ProductId)

		SELECT psam.ProductId FROM [dbo].[Product_SpecificationAttribute_Mapping] psam where psam.SpecificationAttributeOptionId IN (Select SpecificationAttributeOptionId From #SpecialExcludeSpecIds)
        --Special Exclude end

        CREATE TABLE #B2BProductIdsByFacetFilter
        (
            Id int not null
        )
        INSERT INTO #B2BProductIdsByFacetFilter(Id)
        ...
        Where p.Id NOT IN (Select ProductId From #SpecialExcludedProductsIds)

        Set @PercentageOfStockAllowed = (Select PercentageOfStockAllowed From [dbo].[Erp_Account] Where Id = @B2BAccountId)
    END
    -- b2bb2c custom code end
	...
	BEGIN
        ...
        SELECT p.Id FROM Product p with (NOLOCK)
        WHERE '
        ...
        SET @sql = @sql + 'UNION
        SELECT lp.EntityId FROM LocalizedProperty lp with (NOLOCK)
        WHERE lp.LocaleKeyGroup = N''Product''
        ...
        BEGIN
            SET @sql = @sql + 'UNION
            SELECT pptm.Product_Id FROM Product_ProductTag_Mapping pptm with(NOLOCK)
            INNER JOIN ProductTag pt with(NOLOCK) ON pt.Id = pptm.ProductTag_Id
            WHERE pt.[Name] = @OriginalKeywords '
            
            SET @sql = @sql + 'UNION
            SELECT pptm.Product_Id FROM LocalizedProperty lp with (NOLOCK)
            INNER JOIN Product_ProductTag_Mapping pptm with(NOLOCK) ON lp.EntityId = pptm.ProductTag_Id
            WHERE lp.LocaleKeyGroup = N''ProductTag''
            AND lp.LanguageId = ' + ISNULL(CAST(@LanguageId AS NVARCHAR(max)), '0') + '
            AND lp.LocaleKey = N''Name''
	...
	FROM [dbo].[nop_splitstring_to_table](@CategoryIds, ',')
	...
	SELECT CAST(data AS INT)
	FROM [dbo].[nop_splitstring_to_table](@FilteredSpecs, ',')
	...
	SELECT CAST(data AS INT)
	FROM [dbo].[nop_splitstring_to_table](@FilteredProductVariantAttributes, ',')
	...
	SELECT CAST(data AS INT)
	FROM [dbo].[nop_splitstring_to_table](@FilteredManufacturers, ',')
	...
	SELECT CAST(data AS INT)
	FROM [dbo].[nop_splitstring_to_table](@FilteredVendors, ',')
	...
	SELECT CAST(data AS INT)
	FROM [dbo].[nop_splitstring_to_table](@AllowedCustomerRoleIds, ',')

	DECLARE @FilteredCustomerRoleIdsCount INT

	SET @FilteredCustomerRoleIdsCount = (
			SELECT COUNT(1)
			FROM #FilteredCustomerRoleIds
			)
    ...
    SET @sql = 'INSERT INTO #DisplayOrderTmp ([ProductId], [ChildProductId])
    SELECT p.Id, ISNULL(cp.Id, 0)
    FROM Product p with (NOLOCK)
    LEFT JOIN Product cp with (NOLOCK) ON p.Id = cp.ParentGroupedProductId'
    ...
    BEGIN
        ...
        BEGIN  
            SET @sql = @sql + '  LEFT JOIN Erp_Special_Price b2bpp with (NOLOCK)  ON p.Id = b2bpp.NopProductId'  
        END 
        else
        BEGIN  
            SET @sql = @sql + '  LEFT JOIN Erp_Group_Price b2bpgp with (NOLOCK)  ON p.Id = b2bpgp.NopProductId'  
        END 
    END 
    -- b2bb2c custom code end
    ...
    BEGIN
        If @UsePriceGroupPricing = 0 
        BEGIN  
            SET @sql = @sql + '  AND  b2bpp.ErpAccountId = ' + CAST(@B2BAccountId AS nvarchar(max))  
        END 
        else
        BEGIN  
            SET @sql = @sql + '  AND  b2bpgp.ErpNopGroupPriceCodeId = '  + CAST(@PriceGroupCodeId AS nvarchar(max))  
        END 
    END 
    -- b2bb2c custom code end
    ...
    BEGIN
        ...
        -- for b2b stock check by sales org specific warehouses
        IF @IsB2BAccount = 1 AND @PercentageOfStockAllowed < 100
        BEGIN
            SET @sql = @sql + ' AND [pwi].WarehouseId IN ( SELECT NopWarehouseId FROM [dbo].[Erp_Warehouse_Sales_Org_Map] where ErpSalesOrgId = ' + CAST(@B2BSalesOrganisationId AS nvarchar(max))+')
            AND [pwi].StockQuantity > 0 AND [pwi].StockQuantity > [pwi].ReservedQuantity AND [dbo].[ErpPercentageOfStock](' + CAST(@B2BAccountId AS nvarchar(max)) + ',' + CAST(@PercentageOfStockAllowed AS nvarchar(max)) + ', [pwi].ProductId, [pwi].StockQuantity) > 0)'
        END
        ELSE IF @IsB2BAccount = 1
        BEGIN
            SET @sql = @sql + ' AND [pwi].WarehouseId IN ( SELECT NopWarehouseId FROM [dbo].[Erp_Warehouse_Sales_Org_Map] where ErpSalesOrgId = ' + CAST(@B2BSalesOrganisationId AS nvarchar(max))+')
    ...
    BEGIN
        ...
        ELSE
        BEGIN
            SET @sql = @sql + '  AND (  (  cp.Id IS NULL AND ([dbo].ErpProductPrice(' + CAST(@B2BAccountId AS nvarchar(max))+', ' + CAST(@UsePriceGroupPricing AS nvarchar(max))+', +
                    ' + CAST(@PriceGroupCodeId AS nvarchar(max))+', p.Id) >= ' + CAST(@PriceMin AS nvarchar(max)) + ')  )  
                    OR  (  ([dbo].ErpProductPrice(' + CAST(@B2BAccountId AS nvarchar(max))+', ' + CAST(@UsePriceGroupPricing AS nvarchar(max))+', +
    ...
    IF @FilteredCustomerRoleIdsCount > 0
    BEGIN
        SET @sql = @sql + ' AND (p.SubjectToAcl = 0 OR EXISTS (
            SELECT 1 FROM #FilteredCustomerRoleIds [fcr]
            WHERE [fcr].CustomerRoleId IN (
    ...
    IF @StoreId > 0
    BEGIN
        SET @sql = @sql + ' AND (p.LimitedToStores = 0 OR EXISTS (
    ...
    BEGIN
        SET @sql = @sql + ' AND (
            (SELECT AttributesCount FROM #FilteredSpecificationAttributesToProduct fsatp
    ...
    BEGIN
        SET @sql = @sql + ' AND (
            ...
            WHERE (cp.Id IS NULL AND p.Id = fpatp.ProductId) OR cp.Id = fpatp.ProductId) = ' + CAST(@ProductAttributesCount AS NVARCHAR(max)) + ')'
    END
    ...
    END
    -- b2bb2c custom code start
    -- b2b pre filter facet
    IF @IsB2BAccount = 1
    BEGIN
        SET @sql = @sql + ' AND p.Id IN (Select Id from #B2BProductIdsByFacetFilter)'
    END
    -- b2bb2c custom code end
	...
	BEGIN
		...
		INSERT INTO #FilterableSpecs (
			[ProductId]
			,[SpecificationAttributeOptionId]
			)
		SELECT DISTINCT [psam].ProductId
			,[psam].SpecificationAttributeOptionId
		FROM [Product_SpecificationAttribute_Mapping] [psam] WITH (NOLOCK)
		WHERE [psam].[ProductId] IN (
				SELECT [pi].ProductId
				FROM #PageIndex [pi]
				)
			AND [psam].[AllowFiltering] = 1
		...
		CREATE TABLE #FilterableProductVariantIds (
			[ProductId] INT NOT NULL
			,[ProductVariantAttributeId] INT NOT NULL
			)
		...
		INSERT INTO #FilterableProductVariantIds (
			[ProductId]
			,[ProductVariantAttributeId]
			)
		SELECT DISTINCT [ppm].ProductId
			,[ppm].Id
		FROM [Product_ProductAttribute_Mapping] [ppm]
		INNER JOIN #PageIndex [pi] ON [pi].ProductId = [ppm].[ProductId]
			OR [pi].ChildProductId = [ppm].ProductId
		...
		SELECT @FilterableProductVariantAttributeIds = COALESCE(@FilterableProductVariantAttributeIds + ',', '') + CAST(ProductVariantAttributeId AS NVARCHAR(4000))
		FROM #FilterableProductVariantIdsDistinct
		...
		FROM #FilterableManufacturersDistinct
		...
		INSERT INTO #FilterableVendorsDistinct ([VendorId])
    ...
    BEGIN
        ...
        BEGIN
            ...
            AND [pwi].WarehouseId IN ( SELECT NopWarehouseId FROM [dbo].[Erp_Warehouse_Sales_Org_Map] where ErpSalesOrgId = @B2BSalesOrganisationId)  
            AND [pwi].StockQuantity > 0 AND [pwi].StockQuantity > [pwi].ReservedQuantity  AND [dbo].[ErpPercentageOfStock](@B2BAccountId, @PercentageOfStockAllowed, [pwi].ProductId, [pwi].StockQuantity) >= 1) AND cp.UseMultipleWarehouses = 1)))))))  
            BEGIN  
        ...
        BEGIN
            IF EXISTS (SELECT NULL FROM Product p   LEFT JOIN Product cp ON p.Id = cp.ParentGroupedProductId   
            INNER JOIN #PageIndexDistinct [pid] ON [pid].ProductId = p.Id  WHERE (  (cp.ID IS NULL  AND   (  (p.ManageInventoryMethodId = 0) OR  (P.ManageInventoryMethodId = 1 AND  (  (p.StockQuantity > 0 AND p.UseMultipleWarehouses = 0) OR   
            (EXISTS(SELECT 1 FROM ProductWarehouseInventory [pwi] WHERE [pwi].ProductId = p.Id AND [pwi].StockQuantity > 0 AND [pwi].StockQuantity > [pwi].ReservedQuantity AND [dbo].[ErpPercentageOfStock](@B2BAccountId, @PercentageOfStockAllowed, [pwi].ProductId, [pwi].StockQuantity) >= 1) AND p.UseMultipleWarehouses = 1)  )  )  )  )  OR  (p.Id IS NOT NULL 
            AND   (  (cp.ManageInventoryMethodId = 0) OR  (cp.ManageInventoryMethodId = 1 AND  (  (cp.StockQuantity > 0 AND cp.UseMultipleWarehouses = 0) OR   
            (EXISTS(SELECT 1 FROM ProductWarehouseInventory [pwi] 
	...
	DROP TABLE #B2BProductIdsByFacetFilter
    ...
    IF (EXISTS (SELECT * 
            ...
            WHERE TABLE_NAME = 'SpecialExcludedProductsIds'))
    BEGIN
        DROP TABLE #SpecialExcludedProductsIds
    END

    IF (EXISTS (SELECT * 
                FROM INFORMATION_SCHEMA.TABLES 
                WHERE TABLE_NAME = 'SpecialExcludeSpecIds'))
    BEGIN
        DROP TABLE #SpecialExcludeSpecIds
    END

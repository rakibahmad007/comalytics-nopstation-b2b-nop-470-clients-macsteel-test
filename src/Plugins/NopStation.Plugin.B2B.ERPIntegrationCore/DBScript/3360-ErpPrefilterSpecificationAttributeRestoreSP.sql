...
CREATE OR ALTER PROCEDURE [dbo].[ErpPrefilterSpecificationAttributeRestoreSP]
...
BEGIN
	...
	BEGIN
      ...
      -- for all sales org warehouses
		...
		SELECT DISTINCT P.Id from dbo.Product P WITH (NOLOCK) WHERE P.[Deleted] = 0 AND 
		P.Id IN (SELECT [ProductId] FROM dbo.Erp_Special_Includes_And_Excludes WHERE [Id] IN (SELECT [Id] FROM @ids))  
